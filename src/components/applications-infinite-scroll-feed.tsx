...
export default function ApplicationsInfiniteScrollFeed({
  dis = 0,
}: {
  dis?: number;
}) {
  ...
  useEffect(() => {
    if (
      actionState &&
      actionState.additional.applications.length &&
      isPending == false
    ) {
      const additional =
        actionState.additional as ApplicationsActionStateAdditionalType;
